...
# NOTE: This is the legacy generic server implementation.
# The main server.py has been refactored to use native model adapters (MVP).
# To run this legacy server, use: uv run python server_generic.py
...
@contextmanager
def patch_fibo_loading(quantization_level=None):
    original_get_components = FIBOWeightDefinition.get_components
    original_load_weights = FIBOInitializer._load_weights

    def patched_get_components():
        components = original_get_components()
        for c in components:
            c.loading_mode = "mlx_native"
            c.mapping_getter = None
        return components

    def patched_load_weights(model_path):
        weights = original_load_weights(model_path)
        if quantization_level is not None:
             weights.meta_data.quantization_level = quantization_level
        return weights

    try:
        FIBOWeightDefinition.get_components = patched_get_components
        FIBOInitializer._load_weights = patched_load_weights
        yield
    finally:
        FIBOWeightDefinition.get_components = original_get_components
        FIBOInitializer._load_weights = original_load_weights
...
low_ram = False # Global flag for low-ram mode
...
MODEL_REGISTRY = {
    ...
    "briaai/Fibo-mlx-4bit": {"loader": "fibo", "steps": 25, "quantize": 4},
    "briaai/Fibo-mlx-8bit": {"loader": "fibo", "steps": 25, "quantize": 8},
...
def load_model(model_name: str, quantize: int | None, custom_base_path: str | None = None):
    ...
    if loader == "fibo":
        if "mlx" in model_name.lower():
            # For MLX models, we assume native loading and enforce the quantization level from registry/args
            # This ensures mflux treats them as pre-quantized and converts the model structure before loading
            with patch_fibo_loading(quantization_level=effective_quantize):
...
def generate_with_model(instance, model_name: str, task, init_image_path):
    ...
    print(f"Generation parameters: steps={steps}, guidance={guidance}, height={task['height']}, width={task['width']}, seed={common_kwargs['seed']}")
    print(f"Prompt: {prompt}")

    # Set global random seed to ensure deterministic behavior for implicit random generation
    mx.random.seed(common_kwargs['seed'])
...
def compute_image_task():
    ...
    while True:
        ...
        for task in tasklist:
            ...
            if low_ram:
                 # Activate VAE tiling for this generation
                 current_model_instance.tiling_config = TilingConfig()
            else:
                 # Ensure tiling is disabled if low_ram is false
                 current_model_instance.tiling_config = None
...
def main():
    ...
    parser.add_argument('--low-ram', action='store_true', help='Enable Low-RAM mode (automatically activates VAE tiling)')
